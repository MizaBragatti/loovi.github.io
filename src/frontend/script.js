...
// Toggle verbose debug logging for calculation flows (set true to enable)
...
async function calculateMensalidades(estado, valorFipe, placa = null, tipoVeiculo = null) {
  ...
  if (valorFipe > 0 && valorFipe <= 10000) {
    ...
    if (DEBUG_CALC) {
      console.log('[DEBUG_CALC] computeMensalidadesFromCache:', { estado, valorFipe, placa, tipoVeiculo });
      console.log('[DEBUG_CALC] plano escolhido (from cache):', plano?.idPlano);
      console.log('[DEBUG_CALC] looviFipe / indice:', looviFipe, indice);
      console.log('[DEBUG_CALC] mensalidades sample:', mensalidades.slice(0,6));
      console.log('[DEBUG_CALC] valorMensalidade (essencial/semVidro/completo):', valorMensalidadeEssencial, valorMensalidadeSemVidro, valorMensalidadeCompleto);
      console.log('[DEBUG_CALC] valorAtivacao:', valorAtivacao);
      console.log('[DEBUG_CALC] totalAnual (essencial/semVidro/completo):', totalAnualEssencial, totalAnualSemVidro, totalAnualCompleto);
    }
  ...
  return resultado;
    ...
    // this.main("SP"); // Call main with default state - removed
        ...
        attachResizeListener() {
         ...
         getCategoriaFipe(fipeValue) {
          ...
          async calculaMensalidade() {
              ...
              // Parcela mensal equivalente do pagamento anual (12x sem juros)
        ...
        // Habilita/desabilita checkboxes auxiliares (vendedor, plano, tipo de veículo)
        updateCheckboxStates() {
          const enabled = !!this.estado; // true se estado definido
          // seller checkboxes
         ...
         tipoVeiculoCheckboxData() {
                   ...
                   if (suvCheckbox) {
                     suvCheckbox.addEventListener('change', async () => {
                       ...
                       // Se não há estado selecionado, definir SP como padrão internamente e marcar visualmente
                       if (!this.estado || this.estado === 'undefined') {
                         this.estado = 'SP';
                       ...
                       // Atualiza habilitação de outros checkboxes e recalcula imediatamente
      ...
      checkboxData() {
        this.checkboxes.forEach(checkboxNow => {
          checkboxNow.addEventListener('change', async () => {
            ...
            // Se há estado selecionado e SUV/utilitário marcado, recalcular também
            const suvCheckbox = document.getElementById('suv');
            const utilCheckbox = document.getElementById('util');
